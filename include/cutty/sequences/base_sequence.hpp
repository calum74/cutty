...
template <typename T, typename Derived, typename Stored = Derived> class base_sequence
{
  public:
    ...
    virtual_sequence<T, Stored> make_virtual() const
    {
        return {self()};
    }

    operator virtual_sequence<T, Stored>() const
    {
        return make_virtual();
    }
    ...
    {
        size_type c = 0;
        for (auto i = self().first(); i; i = self().next())
            ++c;
        return c;
    }
    ...
    template <typename Predicate> where_sequence<T, Stored, Predicate> where(Predicate p) const
    {
        return {self(), p};
    }

    template <typename Fn> select_sequence<T, Stored, Fn> select(Fn fn) const
    {
        return {self(), fn};
    }
    ...
    template <typename Aggregate> T aggregate(Aggregate agg) const
    {
        T result = {};
    ...
    template <typename Aggregate, typename U> U aggregate(U result, Aggregate agg) const
    {
        for (const value_type *i = self().first(); i; i = self().next())
            result = agg(result, *i);
        return result;
    ...
    template <typename Aggregate, typename U> U accumulate(U result, Aggregate agg) const
    {
        for (const value_type *i = self().first(); i; i = self().next())
            agg(result, *i);
        return result;
  ...
  private:
    ...
    template <typename U> struct asFn
    {
  ...
  public:
    template <typename U> select_sequence<T, Derived, asFn<U>> as() const
    {
        return {self(), {}};
    }
    ...
    template <typename Predicate> size_type count(Predicate p)
    {
        return where(p).size();
    }
    ...
    template <typename Predicate> take_while_sequence<Stored, Predicate> take_while(Predicate p) const
    {
        return {self(), p};
    }

    template <typename Predicate> skip_until_sequence<Stored, Predicate> skip_until(Predicate p) const
    {
        return {self(), p};
    }
    ...
    template <typename Container> void write_to(Container &c) const
    {
        for (auto &i : *this)
            c.insert(c.end(), i);
    }
    ...
    template <typename Container> Container make()
    {
        return {begin(), end()};
    }
