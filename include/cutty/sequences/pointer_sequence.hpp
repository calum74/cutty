...
namespace cutty
{
template <typename T> class pointer_sequence : public sequences::base_sequence<T, pointer_sequence<T>>
{
    const T *a, *b, *current;
  ...
  public:
    ...
    pointer_sequence(const sequences::stored_sequence<Container> &seq)
        : a(seq.container.data()), b(seq.container.data() + seq.container.size())
    {
    }
