...
class generated_sequence
    : public base_sequence<typename helpers::deduce_param<First>::type, generated_sequence<First, Next>>
{
    First firstFn;
    Next nextFn;
...
template <typename Fn> cutty::sequences::generated_sequence<Fn, Fn> generator(Fn fn)
{
    return {fn, fn};
}

template <typename F1, typename F2> cutty::sequences::generated_sequence<F1, F2> generator(F1 f1, F2 f2)
{
    return {f1, f2};
}
