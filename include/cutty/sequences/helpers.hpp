...
template <typename Fn> struct deduce_param : public deduce_param<decltype(&Fn::operator())>
{
};

template <typename R, typename C, typename T> struct deduce_param<R (C::*)(T) const>
{
...
template <typename R, typename C, typename... Args> struct deduce_result<R (C::*)(Args &&...) const>
{
...
template <typename R, typename... Args> struct deduce_result<R(Args...)>
{
    typedef typename remove_all<R>::type type;
};
...
template <typename P> struct project_first;

template <typename T1, typename T2> struct project_first<std::pair<T1, T2>>
{
    typedef typename remove_all<T1>::type type;
    const type &operator()(const std::pair<T1, T2> &p) const
    {
        return p.first;
    }
};
...
template <typename P> struct project_second;

template <typename T1, typename T2> struct project_second<std::pair<T1, T2>>
{
