...
ancy::button::button(widget &parent, position p, size s, char32_t key, std::string text, const style &normal, const style &selected, std::function<void()> action) : widget(parent, p, s), m_key(key), m_normal(normal), m_selected(selected), m_action(action)
{
    set_text(text);
}
...
{
    return p.x >= m_position.x && p.x < m_position.x + m_size.w &&
        p.y >= m_position.y && p.y < m_position.y + m_size.h;
}
