...
void print(const cy::sequence<int> &s)
{
    for (auto i : s)
        std::cout << i;
    std::cout << std::endl;
...
int main()
{
    ...
    print(seq1 + cy::seq(10, 19));
    print(cy::list(1, 2, 3) + cy::list(4));

    // merge(s,fn) merge two sequences, calling fn on each pair
    print(seq1.merge(cy::seq(10, 19), [](int a, int b) { return a + b; }));

    // Cast each element to a new type using as<T>()
    print(cy::list(true, false).as<int>());

    // Repeat sequence a number of times
    print(cy::list(1, 2).repeat(3));
