...
int main()
{
    auto s = cy::list("a", "b", "c");
    auto e = cy::seq<const char*>();
    ...
    std::cout << (s.as<std::string>() == cy::list("a","b","c").as<std::string>()) << std::endl;

    // The equals() function allows you to specify a comparator function
    std::cout << s.equals(cy::list("a", "b", "c"), [](const char * s1, const char *s2) { return std::strcmp(s1,s2)==0; }) << std::endl;

    // Other comparators work fine as well
    std::cout << (s.as<std::string>() < cy::list("a","b","c").as<std::string>()) << std::endl;

    // lexographical_compare allows you to specify a comparator function explicitly
    std::cout << s.lexographical_compare(cy::list("a", "b", "c"), [](const char * s1, const char *s2) { return std::strcmp(s1,s2)<0; }) << std::endl;
    ...
    int hash = cy::list(1,2,3).aggregate(0, [](int n1, int n2) { return n1*13 + n2; });
